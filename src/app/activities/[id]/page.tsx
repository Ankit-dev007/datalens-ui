...
export default function ActivityDetailsPage() {
    ...
    useEffect(() => {
        if (!id) return;
        const activityId = Array.isArray(id) ? id[0] : id; // Handle potential array from Next.js

        Promise.all([
          // Note: api.getActivities() returns list. We need a single get. 
          // Current api.ts doesn't have a single get method exposed widely or the endpoint calls "getActivity" internally?
          // The backend route '/activities/:id' exists (GET).
          // But api.ts only has 'getActivities' which fetches all.
          // I should add getActivity(id) to api.ts, but for now I can filter manual since I don't want to break flow.
          // Wait, I can try to fetch all and find, OR add the method.
          // Adding method is cleaner.
          // But I'll use fetchJson directly if needed or just add it to api.ts in next step.
          // Let's assume I add `getActivity` to api.ts.
          fetch(`/api/activities/${activityId}`).then(res => res.json()), // Direct fetch for now as fallback
